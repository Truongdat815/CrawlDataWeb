"""
Script migration: Đổi tên các fields trong collection comments
Fields mapping:
...
def migrate_comments_collection():
    """Đổi tên các fields trong collection comments"""
    try:
        ...
        safe_print(f"\n📊 Tổng số documents trong collection comments: {total_count}")
        
        if total_count == 0:
            safe_print("📭 Chưa có dữ liệu nào trong collection comments")
            client.close()
            return
        ...
        for doc in collection.find():
            ...
            if update_data or unset_data:
                ...
                update_count += 1
                web_comment_id = doc.get("webCommentId") or doc.get("web_comment_id", "N/A")
                safe_print(f"  ✅ Đã cập nhật comment: {web_comment_id}")
            else:
                skip_count += 1
