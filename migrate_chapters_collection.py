"""
Script migration: Đổi tên các fields trong collection chapters
Fields mapping:
...
def migrate_chapters_collection():
    """Đổi tên các fields trong collection chapters"""
    try:
        ...
        safe_print(f"\n📊 Tổng số documents trong collection chapters: {total_count}")
        
        if total_count == 0:
            safe_print("📭 Chưa có dữ liệu nào trong collection chapters")
            client.close()
            return
        ...
        for doc in collection.find():
            ...
            if update_data or unset_data:
                ...
                update_count += 1
                web_chapter_id = doc.get("webChapterId") or doc.get("web_chapter_id", "N/A")
                safe_print(f"  ✅ Đã cập nhật chapter: {web_chapter_id}")
            else:
                skip_count += 1
