"""
Script migration: Đổi tên các fields trong collection scores
Fields mapping:
- score_id -> scoreId
...
def migrate_scores_collection():
    """Đổi tên các fields trong collection scores"""
    try:
        ...
        safe_print(f"\n📊 Tổng số documents trong collection scores: {total_count}")
        
        if total_count == 0:
            safe_print("📭 Chưa có dữ liệu nào trong collection scores")
            client.close()
            return
        ...
        for doc in collection.find():
            ...
            if update_data or unset_data:
                ...
                update_count += 1
                score_id = doc.get("scoreId") or doc.get("score_id", "N/A")
                safe_print(f"  ✅ Đã cập nhật score: {score_id}")
            else:
                skip_count += 1
